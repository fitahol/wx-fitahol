...
Page({
	...
	searchFocus() {
		...
		Fetch(`/account/member/card/?user_id=${data.self}`)
		.then((res) => {
			res.data.forEach((item) => {
				item.searchItem = `${item.user_id} - ${item.nickname} `
			})
	...
	handleSearchItem(e) {
		this.setData({
			searchResult: e.target.dataset.search,
			searchResultId: e.target.dataset.id,
			searchResultList: []
		})
	},
	// 开始日期
	...
	buildCourseConfirm() {
		...
		if (!data.searchResultId) {
			...
			return
		} else if (!~['00', '30'].indexOf(data.startTime.split(':')[1])) {
			wx.showModal({
	...
	onLoad() {
		...
		const hour = +wx.getStorageSync('hour')
