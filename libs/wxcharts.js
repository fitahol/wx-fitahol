		...
		//柱状图
				...
				function chartSet() {
						...
						for (var i = 0; i < totalBars; i++) {
								...
								var barVal = +parseInt(arrVal[1]) + 1;
				...
				function drawChartWithAnimation() {
						...
						for (var i = 0; i < totalBars; i++) {
								...
								if (!bVal) {
									// 数值为0，展示背景灰色
									drawRectangle(bX, bY, bWidth, -100, params.nodataBgcolor || '#edf0f5');
								} else {
									drawRectangle(bX, bY, bWidth, bHt);
								}
								// draw(bX,bY,bWidth,maxValue-bHt);
								cxt.setFillStyle("#000000");//singer bar number color
								if (bVal.toString().length === 1) {
									cxt.fillText(bVal,bX + 6,textY-10);//singer bar number
								} else {
				...
				function drawRectangle(x, y, w, h, bgColor) {
						cxt.beginPath();
						cxt.rect(x, y, w, h);
						cxt.setStrokeStyle("#ffffff");//single bar border color
						cxt.setFillStyle(bgColor || bgColors);//single bar bgcolor
						cxt.stroke();
						cxt.fill();
						cxt.closePath();
				}
